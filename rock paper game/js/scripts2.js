...
const resetGameUI = () => {
  gameContentElement.classList.remove(
    ...
    "gameContent--shake" // Added from scripts2.js
  );
...
const showResult = async (userChoice, computerChoice) => {
  ...
  if (userChoice === computerChoice) {
    ...
    playerScore++;
    // Deduct 1 from computer if computer's score > 0
    if (computerScore > 0) {
      computerScore--;
    }
    ...
    computerScore++;
    // Deduct 1 from player if player's score > 0
    if (playerScore > 0) {
      playerScore--;
    }
    computerScoreElement.textContent = computerScore;
...
const startCountdown = () => {
  ...
  if (countdown >= 0) {
    ...
    if (!selectedGameChoiceElement) {
      isGameActive = false;
      countdown = 3; // Reset countdown (from scripts2.js)
      return;
    }
